...
bad_inc = own[(own['ACTION'].astype(str).str.upper()=='INCREASE') & (own['P&L %'] < -0.02)] if 'P&L %' in own.columns else pd.DataFrame()
...
book_rows = own[own['BOOK %'].notna()] if 'BOOK %' in own.columns else pd.DataFrame()
profitable = own[own['P&L %'] > 0.0] if 'P&L %' in own.columns else pd.DataFrame()
print(f"\nP-01/02/03/04/05  Profit booking plan:     {len(book_rows)} stocks have booking plan")
if len(book_rows):
    for _, r in book_rows.iterrows():
        when = r.get('WHEN', '')
        print(f"    {r['symbol']:<12} Book {r['BOOK %']:.0%}  PnL={r['P&L %']:+.1%}  {when}")
else:
...
loss_sells = own[(own['ACTION'].astype(str).str.upper().str.contains('SWAP|SELL')) & (own['P&L %'] < -0.05)] if 'P&L %' in own.columns else pd.DataFrame()
ml_guarded = loss_sells[loss_sells.get('ML', pd.Series()).isin(['HOLD','STRONG_BUY'])] if len(loss_sells) else pd.DataFrame()
print(f"\nR-04         Loss rotation without ML guard: {len(loss_sells)-len(ml_guarded)} unprotected")

# C-03/C-04: EXHAUSTION surfaced and actioned (EXIT_SCORE column removed in new format)
print("\nC-03/C-04    EXIT_SCORE column: REMOVED (skip)")
