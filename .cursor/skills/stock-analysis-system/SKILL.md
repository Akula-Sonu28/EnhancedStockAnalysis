...
**Multi-desk research layer:** For investment narrative on pipeline output (fundamental/technical/macro/portfolio framing), use `india-markets-orchestrator` and its team skills under `.cursor/skills/india-markets-*`. **Engineering layer:** `software-product-orchestrator` and roles under `.cursor/skills/software-product-*` for BUILD/SHIP. Use **this skill** for code/config/contracts; use **five-agent-council** for system changes (see `five-agent-council/references/product-teams-bridge.md`).
