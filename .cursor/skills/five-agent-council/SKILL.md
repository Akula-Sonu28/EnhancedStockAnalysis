...
description: >-
  Five-expert multi-agent council for Stock_Analysis — coordinated debate across
  architecture, quant/scoring, risk, contracts/QA, and adversarial review.
  Runs multiple rounds until consensus or documented trade-offs, then requires
  peer acknowledgment before any system deployment. Use when the user asks for
  multi-agent review, expert council, agent debate, peer sign-off, architectural
  decisions, scoring/risk changes, or says COUNCIL, DEBATE, or PEER REVIEW for
  this repo.
---
...
Before Round 1, read `.cursor/skills/stock-analysis-system/SKILL.md` yourself
(orchestrator). Each subagent reads its persona + stock-analysis-system.
...
4. Round-1 instructions from [reference.md](reference.md#round-1-position-template)
